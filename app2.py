...
if 'Teste' in df['Marca'].values:
    df.drop(df[df['Marca'] == 'Análise'].index)
...
with col1:
    ...
    # Adiciona uma linha
    df.loc[len(df)] = ['Análise', sensibilidade_int, 'Análise', resultado, 1, 'Análise', tempo_reverberacao_input, volumetria_input, saude_input/100, random.random()]
...
with col2:
    ...
    base = alt.Chart(df).mark_circle(size=180).encode(
        ...
        alt.Y('Random',  title='', axis=alt.Axis(
            tickCount=6, format='d', titleFont='Arial', titleFontSize=13, titleColor='white', gridColor='white')),
